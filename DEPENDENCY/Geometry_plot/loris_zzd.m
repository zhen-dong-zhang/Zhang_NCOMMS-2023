function loris_zzd(N,eo,lon,lat,sc,fax,opt,vlon,vlat,vrad,vmag,vcol,dcol,...
...
if sc==0
  ...
  vert=[x(:,:,mf)' y(:,:,mf)' z(:,:,mf)'];
  %%%chunck region
%   pm=plot3(horz(:,1:N),horz(:,N+1:2*N),horz(:,2*N+1:3*N),'--','Color', [0.75 0.75 0.75]);
%   um=plot3(vert(:,1:N),vert(:,N+1:2*N),vert(:,2*N+1:3*N),'--','Color', [0.75 0.75 0.75]);
...
%   %%%% ADD by zzd anomly %%%%%%
     ...
     [xp4,yp4,zp4]=sph2cart(wed1(7)*pi/180,wed1(8)*pi/180,1);
% 
%      tm=plot3([xp1, xp2],[yp1, yp2],[zp1, zp2],':k');
%      hold on
%      tm=plot3([xp2, xp3],[yp2, yp3],[zp2, zp3],':k');
%      hold on
%      tm=plot3([xp3, xp4],[yp3, yp4],[zp3, zp4],':k');
%      hold on
%      tm=plot3([xp4, xp1],[yp4, yp1],[zp4, zp1],':k');
%      hold on
%      
%      
%      [xp1,yp1,zp1]=sph2cart(wed2(1)*pi/180,wed2(2)*pi/180,1);
%      [xp2,yp2,zp2]=sph2cart(wed2(3)*pi/180,wed2(4)*pi/180,1);
%      [xp3,yp3,zp3]=sph2cart(wed2(5)*pi/180,wed2(6)*pi/180,1);
% 
%      tm=plot3([xp1, xp2],[yp1, yp2],[zp1, zp2],'--k');
%      hold on
%      tm=plot3([xp1, xp3],[yp1, yp3],[zp1, zp3],'--k');
%      hold on
%      tm=plot3([xp3, xp2],[yp3, yp2],[zp3, zp2],'--k');
%      hold on
...
%    for ist =1:length(vlon_st)
%       [xvv,yvv,zvv]=sph2cart(vlon_st(ist)*pi/180,vlat_st(ist)*pi/180,vrad_st(ist));
%       dm=scatter3(xvv,yvv,zvv,5.0,'v','b');
%       hold on
%       ist
%    end
  
%     for ieq =1:length(vlon)
%       %[~,~,mf]=sphere2cube(lon*0.2,lat);
%       %cell2mat(dcol{vcol(ieq)})
%       [xvv,yvv,zvv]=sph2cart(vlon(ieq)*pi/180,vlat(ieq)*pi/180,vrad(ieq));
%       dm=scatter3(xvv,yvv,zvv,vmag(ieq)*20.0,'filled','MarkerFaceColor',dcol{vcol(ieq)});
%       hold on
%       ieq
%     end
