...
hold on
plotm(plat,plon,'c','Linewidth',1)
 hold on
 
%%stations
 locuniq=unique([stlat(1:sum(stnum(1:ieq)))', stlon(1:sum(stnum(1:ieq)))'],'rows');
 plotm(locuniq(:,1),locuniq(:,2),'v','Color',[17 17 17]/255,'Linewidth',1,'MarkerSize',eqmag(1))
 hold on
 
 for i=1:ieq
    plotm(eqlat(i),eqlon(i),'-o','MarkerFaceColor',dcol{vcol(i)},'MarkerEdgeColor',dcol{vcol(i)},'MarkerSize',0.8*eqmag(i))
    hold on
 end
 
set(gca, 'fontsize',20)
setm(gca, 'FontSize', 20,'FLineWidth',1,'Grid','on','MapProjection','robinson');
%%%standalone legend
...
icons(6).MarkerSize = 9;
icons(6).LineWidth = 1.0;
icons(8).MarkerSize = 25;
icons(10).MarkerSize = 28;
icons(12).MarkerSize = 31;
legend boxoff
hold off
...
%%%calculate areas a = abs(lon1-lon2) .* abs(sin(lat1)-sin(lat2)) / (4*pi);
for ilon = 1:length(yedgs)-1
    for ilat = 1:length(xedgs)-1
        lon1 = yedgs(ilon+1);
        lon2 = yedgs(ilon);
        lat1 = xedgs(ilat+1);
        lat2 = xedgs(ilat);
        ar(ilat,ilon) =  areaquad(lat1,lon1,lat2,lon2,wgs84);
       % ar(ilat,ilon) = pi/180*6371^2*abs(lon1-lon2) .* abs(sin(lat1)-sin(lat2));
    end
end


% fldl=log10(fld./ar);
 fldl = log10(10000.0*fld./ar);
% fldl = 10000.0*fld./ar;
surfm(latbin,lonbin,fldl, 'EdgeColor', 'none')
%  surfm(latbin,lonbin,ar, 'EdgeColor', 'none')

%%%save data .mat 
% save latbin.mat latbin
% save lonbin.mat lonbin
% save fldl.mat fldl
% zlimits = [0 max(max(ar))];
% zlimits = [0 0.2*max(max(fldl))];
...
set(colorTitleHandle ,'String','Log (midpoints per 100x100 km^2)');
